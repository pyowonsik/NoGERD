    ...
    return BlocListener<RecordBloc, RecordState>(
      listener: (context, state) {
        ...
        state.successMessage.fold(
          () {},
          (_) => context.read<InsightsBloc>().add(const InsightsEvent.refresh()),
        );
      },
...
class _SymptomDistribution extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final symptoms = top5.map((dist) {
      final percentage = totalCount > 0 ? ((dist.count / totalCount) * 100).round() : 0;
      return _SymptomData(
        dist.symptom.label,
        percentage,
        ...
        children: [
          Row(
            ...
            children: [
              const Row(
                children: [
                  Icon(Icons.restaurant_rounded, color: AppTheme.mealColor, size: 20),
                  SizedBox(width: 8),
                  Text(
                    '식사 후 증상 발생률',
                    style: TextStyle(
    ...
    return Column(
      ...
      children: [
        ...
        const _InsightCard(
          ...
          title: '식사 패턴',
          message: '저녁 식사 후 증상이 집중돼요. 저녁 식사량을 줄이고 취침 3시간 전에 식사를 마치세요.',
        ...
        _InsightCard(
          ...
          message: state.triggerAnalysis.isNotEmpty
              ? '${state.triggerAnalysis.first.category.label} 섭취 후 증상이 자주 발생해요. 섭취를 줄여보세요.'
              : '식사 기록을 더 남기면 트리거 음식을 분석해드려요.',
          color: AppTheme.warning,
        ),
        const SizedBox(height: 10),
        const _InsightCard(
          ...
          title: '약물 복용',
          message: '규칙적인 약물 복용이 증상 완화에 도움이 됩니다. 복용 시간을 지켜주세요.',
